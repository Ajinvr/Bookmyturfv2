...
function ManagerSignup() {
  ...
  const signup = async () => {
    try {
      const response = await axiosInstance.post('/api/manager/managerSignup', {
        email,
        password
      });
      ...
      setTimeout(() => {
        ...
        if (userrole == 'manager') {
          navigate('/manager/assigneTurfBookings')
        }

        if (userrole == 'admin') {
          navigate('/admin/allTurfBookings')
        }
      ...
      <div className="flex flex-col md:flex-row items-center md:w-3/4 lg:w-1/2 bg-primary p-6 rounded-lg shadow-lg w-10/12">
        ...
        <div className="md:w-1/2 bg-primary p-8 rounded-lg">
          ...
          <div className='text-neutral text-center text-sm mt-2'>
            <Link to='/managerlogin'>Already have an account?</Link>
          </div>
