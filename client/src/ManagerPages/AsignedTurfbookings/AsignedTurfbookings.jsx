...
const fetchAssignedTurfs = async () => {
  const response = await axiosInstance.get('/api/manager/getManagerAssignedTurfsOrders');
  return response.data.assignedTurfsOrders; 
};

function AsignedTurfbookings() {
  
  const { data, isLoading, isError } = useQuery('assignedTurfsOrders', fetchAssignedTurfs);
 ...
 async function cancelBooking(e) {
  ...
  const parentElement = e.target.parentElement;
    ...
    try {
      ...
      let latestdata = await fetchAssignedTurfs()
...
export default AsignedTurfbookings;
